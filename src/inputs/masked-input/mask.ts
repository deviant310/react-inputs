import { escapeSpecialCharacters } from '../../factory/helpers';

import { Some } from '../../factory/utility-types';

class Mask {
  private static definitions: Mask.Definitions = {
    '*': /./,
    '0': /\d/,
    'a': /[a-zA-Z]/,
  };

  private entries: Mask.Entry[];
  private stub: string;

  constructor (public props: Mask.Props) {
    ...
    const { entries } = [...pattern.matchAll(maskRegExpPattern)]
      .map(({ groups }) => Object
        ...
        .find(([, value]) => value !== undefined) as [Mask.EntryType, string],
      )
  ...
  copyWith (props: Some<Mask.Props>) {
...
namespace Mask {
  ...
  export type Definitions = Record<string, RegExp>;
