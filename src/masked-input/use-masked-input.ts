...
export const useMaskedInput = (props: UseMaskedInputProps) => {
  ...
  const mask = useRef(new Mask({ definitions, dirtyValue, pattern, stub }));
  ...
  const restoreCaretPositionFrom = useCallback(
    (position: number, lazy?: boolean) => {
      const caretPosition = mask.current.getRelevantCaretPositionClosestTo(
        position,
        lazy,
      );
  ...
  return useMemo(
    ...
    [inputRef, inputValue, onInputChange, onInputKeyDown, onInputMouseDown],
  );
};
...
export interface UseMaskedInputResult {
