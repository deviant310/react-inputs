...
module.exports = env => {
  ...
  return {
    devServer: {
      ...
      port: devServerPort,
      static: {
        directory: paths.packageOutput,
      },
    },
    devtool: !isProductionBuild && 'source-map',
    entry: isDevServer
      ? paths.packageDemoEntrypoint
      : {
    ...
    output: {
      ...
      path: paths.packageOutput,
    },
    plugins: [
      new ESLintPlugin({
        cacheLocation: '.yarn/.cache/eslint-webpack-plugin/.eslintcache',
      ...
      isDevServer && new HtmlWebpackPlugin({
        template: resolve(paths.packageStatic, 'index.html'),
        title: app.name,
      }),
    ].filter(Boolean),
    resolve: {
