...
import { SelectInputVirtualListProps } from '../types';

export const VirtualList = memo(
  <Element extends HTMLElement, Item>(
    props: SelectInputVirtualListProps<Element, Item>,
  ) => {
    ...
    useEffect(
      () => {
        ...
        wrapperElement?.parentElement
          ?.addEventListener('scroll', renderBatchOfNodes);

        return () => {
          wrapperElement?.parentElement
            ?.removeEventListener('scroll', renderBatchOfNodes);
        };
      },
      [renderBatchOfNodes],
    );
    ...
    return (
      <div ref={wrapperRef} style={wrapperStyle}>
        {Array
          .from(itemsToRender.entries())
          .map(([index, item]) => renderItem(
            item,
            el => setListItemOffsetByElement(index, el),
            index,
          ))
        }
      </div>
